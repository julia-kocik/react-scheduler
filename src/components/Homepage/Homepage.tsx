...
export default function Homepage() {
  ...
  const [forceFetchAfterDelete, setForceFetchAfterDelete] = useState<boolean>(
    true
  );
  const [forceFetchAfterUpdate, setForceFetchAfterUpdate] = useState<boolean>(
    true
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (
          forceFetchAfterPost ||
          forceFetchAfterDelete ||
          forceFetchAfterUpdate
        ) {
        ...
        showToast(
          "Error occurred when data fetching, please try again later.",
          "red",
          setToastInfo
        );
      } finally {
