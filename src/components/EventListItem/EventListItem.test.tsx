...
describe('EventListItem', () => {
  ...
  beforeEach(() => {
    ...
    render(
      <>
        <Toast message='' color=''/>
        <EventListItem setToastInfo={setToastInfo} setForceFetchAfterUpdate={setForceFetchAfterUpdate} setForceFetchAfterDelete={setForceFetchAfterDelete} {...eventListItem} />
      </>
    )
  });
  ...
  test('handles delete event', async () => {
    ...
    await waitFor(() => {
      expect(axios.delete).toHaveBeenCalledWith(`${apiUrl}/api/v1/event/${eventListItem.id}`);
  ...
  test('handles update event', async () => {
    ...
    fireEvent.click(screen.getByText(/Update/));
    fireEvent.change(screen.getByLabelText(/Name/), { target: { value: 'Mary' } });
    fireEvent.click(screen.getByText(/Save/));
    await waitFor(() => {
      expect(axios.patch).toHaveBeenCalledWith(`${apiUrl}/api/v1/event/${eventListItem.id}?name=Mary`);
